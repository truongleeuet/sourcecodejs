/*
 * Adm Image Slider v1.0
 *
 * By Admicro.vn
 * http://admicro.vn
 *
 * Copyright (C) 2015 Admicro.vn
 * @link http://admicro.vn
 * @author kydq
 */

var AdmImageSlider = (function() {
	"use strict";
	...
	var Slider = function(args) {
		...
		this.settings = {
			selector: '',
			width: 1000, // Required but can be set by CSS
			...
			captions:false,
			classes: {
				container: 'adm-image-slider',
				slide: 'adm-slide',
				previousSlide: 'adm-previous-slide',
				currentSlide: 'adm-current-slide',
				nextSlide: 'adm-next-slide',
				previousNav: 'adm-previous-nav',
				nextNav: 'adm-next-nav',
				animating: 'adm-is-animating',
				touchEnabled: 'adm-touch-enabled',
				touching: 'adm-is-touching',
				directionPrevious: 'adm-direction-previous',
				directionNext: 'adm-direction-next'
			},
			onInit: function(){},
			onStart: function(){},
			onStop: function(){},
		...
		_addClass(sliderEl, 'adm-effect-'+ this.settings.effect);
...
/*	Slider.prototype.start = function() {
	...
	Slider.prototype.destroy = function() {
		...
		_removeClass(this._attributes.container, 'adm-effect-'+ this.settings.effect);
	...
	var _updateActiveBullet = function(slider, activeIndex) {
		...
		Array.prototype.forEach.call(bullets, function(bullet, i){
			_removeClass(bullet, 'adm-bullet-active');
			bullet.setAttribute('aria-selected', 'false');
			if(i === activeIndex){
				_addClass(bullet, 'adm-bullet-active');
				bullet.setAttribute('aria-selected', 'true');
			}
		}.bind(this));
	};

	Slider.prototype.addBulletNav = function() {
		_addClass(this._attributes.container, 'adm-has-bullet-nav');

		// Create bullet nav
		var bulletNav = document.createElement('div');
		_addClass(bulletNav, 'adm-bullet-nav');
	...
	Slider.prototype.addCaptions = function() {
		_addClass(this._attributes.container, 'adm-has-captions');

		Array.prototype.forEach.call(this._attributes.slides, function(slide, i){
			var caption = document.createElement('div');
			_addClass(caption, 'adm-caption');

			var captionContent = '';
			if(slide.getAttribute('title')){
				captionContent += '<div class="adm-caption-title">'+ slide.getAttribute('title') +'</div>';
			}
			if(slide.getAttribute('data-caption')){
				...
				if(dataCaption.substring(0,1) == '#' || dataCaption.substring(0,1) == '.'){
					...
					if(external){
						captionContent += '<div class="adm-caption-content">'+ external.innerHTML +'</div>';
					}
				} else {
					captionContent += '<div class="adm-caption-content">'+ slide.getAttribute('data-caption') +'</div>';
				}
			} else {
				if(slide.innerHTML){
					captionContent += '<div class="adm-caption-content">'+ slide.innerHTML +'</div>';
				}
			}
